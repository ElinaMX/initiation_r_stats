...
```{r init, eval=TRUE, include= TRUE, echo=FALSE, cache=TRUE}
options("scipen" = 10)
...
```{r load_local_mapsf, eval=TRUE, include=FALSE}
library(mapsf)  
```

```{r help_pckg,eval=FALSE}
...
B)  **Téléchargez** [**ces données**](https://github.com/HuguesPecout/Initiation_R_stats/raw/main/data/data.zip) de recensement **(INSEE, 2016)** pour les communes de la Martinique en 2011 et 2016, associées à une couche géographique **(IGN - BD ADMIN EXPRESS, 2016)**.
...
**Vérifiez le type de données stocké pour chaque variable**. Si nécessaire, **corrigez la classe de données des variables** '*CODGEO*, '*DEP*', '*REG*', '*CATAEU2010*' **de votre data frame comportant les données de recensement**.  
**N'oubliez pas d'assigner le résultat de la conversion :**

```{r exo_conv_col, eval=FALSE}
...
```{r ex_lower, eval=TRUE}
# tout en minuscule
tolower(text)
```

```{r ex_upper, eval=TRUE}
# tout en majuscule
toupper(text)
...
Indicateurs de la dispersion des valeurs des individus autour de la moyenne. La variance est la moyenne des carrés des écarts à la moyenne. L’écart-type est sa racine carrée. Ce sont des **paramètres de dispersion.** Attention, R calcule la variance en prenant en compte la **correction de Bessel**, en divisant les carrés des écarts à la moyenne par n-1 plutôt que par n, afin de simuler un échantillon.
...
### Les valeurs centrales : Moyenne, Médiane et Mode
...
Les **paramètres de dispersion absolue** se calculent uniquement sur les **variables quantitatives**. Elles permettent de comparer des valeurs **dans la même unité de mesure** (ex : les notes d'un examen entre deux classes).
...
### Les paramètres de dispersions relative

Les paramètres de dispersion relative permettent de comparer les dispersions de deux séries statistiques **d'unités de mesure différentes** (ex : les notes d'un examen et la vitesse observée de véhicules).
...
![](figures/forme_distribution_2.png)
...
Pour discrétiser une série statistique, on peut mobiliser la fonction [classIntervals()]{.fonction} du package [classInt]{.package}.

```{r discr_mapsf, include=FALSE }
# install.packages("classInt")
library(classInt)

ma_discretisation <- classIntervals(var = mtcars$hp,
                                    n = "6",
                                    style = "quantile")
...
Vous pouvez choisir différentes méthodes de discrétisation avec l'argument '*style*'.
...
ma_discretisation <- classIntervals(mtcars$hp, style = "sd")

# Utiliser la fonction cut()
mtcars$hp_classe <- cut(mtcars$hp, 
                        breaks = ma_discretisation$brks, 
                        include.lowest = TRUE, 
                        right = TRUE)
...
abline(v = ma_discretisation$brks, col = "blue", lwd = 2, lty = 2)
...
![](figures/boxplot.png){width="350px"}
...
**La carte permet d'explorer la répartition spatiale d'une distribution d'une variable géographique**. Plusieurs packages permettent de faire des carte avec R, **le package** [mapsf]{.package} **est spécialisé dans la production de cartes thématiques et statistiques** [@mapsf].

![figures/carto.png]{width = 50%}  
*Source : https://riatelab.github.io/mapsf/index.html*
...
mtq_data <- read.csv("data/INSEE_COM_972.csv", header = TRUE,
...
3- Relation entre **deux variables quantitatives** (3.3)
2- Relation entre **une variable qualitative et une variable quantitative** (3.4)\
...
--\> [Le calcul du χ² et du degré de liberté ?](https://github.com/HuguesPecout/Initiation_R_stats/raw/main/data/Khi2.xls)\
